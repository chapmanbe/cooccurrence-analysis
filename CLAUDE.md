...
- **Figure labels must go through `VOCAB`** (`vocabulary.jl`), never a hardcoded noun. Titles and axis labels interpolate `$(VOCAB.item)` / `$(VOCAB.items)` / `$(VOCAB.record)` / `$(VOCAB.records)` / `$(VOCAB.group)`, plus the `item_lc()` / `record_lc()` helpers for mid-sentence use. Callers set the nouns once with `set_vocabulary!(item="Cancer Site", record="Patient", …)`. This is the single place an application's domain terms enter the package, and it affects labels only — never a computation, a column name, or a return value. A hardcoded "Item" in a title silently ships the placeholder into someone's published figure.
- **Anything the caller must know that the package cannot** — which items are exclusive to which group, what a group value means — is a keyword argument (`exclusive_items`, `labels`, `colors`), never baked into source. Defaults preserve neutral behavior; the domain layer supplies the knowledge.
